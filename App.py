...
# + Módulos interactivos para sellos: (1) Aplicabilidad y (2) Tamaño/posición (Tabla 17) con visual 2D tipo etiqueta
# ------------------------------------------------------------------
...
def draw_regular_octagon(ax, cx, cy, side_px, fc='black', ec='white', lw=1.6):
    # Dibuja un octágono regular centrado en (cx, cy) con "side_px" como "diámetro" visual
    r = side_px / 2.0
    pts = []
    for k in range(8):
        ang = math.radians(22.5 + k*45.0)  # rotación para lado plano arriba
        x = cx + r * math.cos(ang)
        y = cy + r * math.sin(ang)
        pts.append((x, y))
...
for categoria, items in CATEGORIAS.items():
    ...
    for item in items:
        ...
        # Presentación mejorada en lugar de bloques largos
        if titulo == "Ubicación y tamaño del sello (Tabla 17)":
            info_card("""
            <b>Objetivo:</b> evaluar disposición y dimensión de sellos de advertencia en la cara principal.<br>
            • Ubicar en el <b>tercio superior</b> de la cara principal, sin obstrucciones.<br>
            • <b>Tamaño mínimo</b> del octágono según área de la cara (Tabla 17).<br>
            • Mantener legibilidad/contraste del texto.
            """)
        else:
            st.markdown(f"**Qué verificar:** {que_verificar}")
            st.markdown(f"**Referencia:** {referencia}")
            st.markown_aplica = st.markdown(f"**Aplica a:** {APLICA.get(titulo, 'Ambos')}")
        ...
        # =========================
        # 4.5 Ubicación y tamaño del sello (Tabla 17) — VISUAL 2D MEJORADO
        # =========================
        if titulo == "Ubicación y tamaño del sello (Tabla 17)":
            with st.expander("Abrir referencia visual (Tabla 17) y disposición de varios sellos"):
                ...
                if lado_cm is None:
                    ...
                    # --------- Visualización 2D "tipo etiqueta" ----------
                    # Escala: 1 cm = 22 px (ligeramente mayor para detalle)
                    scale = 22.0
                    envase_w_cm = max(ancho_total + 3.0, 12.0)  # ancho mínimo
                    envase_h_cm = envase_w_cm * 1.45            # proporción
                    tercio_y = envase_h_cm * (2.0/3.0)

                    fig, ax = plt.subplots(figsize=(7.2, 5.2))

                    # Fondo blanco (la "etiqueta")
                    ax.add_patch(Rectangle((0, 0), envase_w_cm*scale + 40, envase_h_cm*scale + 40,
                                           facecolor="white", edgecolor="white"))

                    # Sombra leve del envase
                    ax.add_patch(Rectangle((20+4, 20-2), envase_w_cm*scale, envase_h_cm*scale,
                                           facecolor="#F0F0F0", edgecolor="#E0E0E0"))
                    # Envase (rectángulo con borde suave)
                    ax.add_patch(Rectangle((20, 20), envase_w_cm*scale, envase_h_cm*scale,
                                           facecolor="#F9F9F9", edgecolor="#CCCCCC", linewidth=1.2))

                    # Línea de guía del tercio superior
                    ax.axhline(20 + tercio_y*scale, linestyle="--", linewidth=1)

                    # Dibujar sellos (octágonos) centrados horizontalmente en el tercio superior
                    total_w_px = ancho_total * scale
                    start_x = 20 + (envase_w_cm*scale - total_w_px) / 2.0 + (lado_cm*scale)/2.0
                    y_center = 20 + tercio_y*scale + (lado_cm*scale)/2.8  # un poco por encima de la línea

                    for i in range(num_sellos):
                        cx = start_x + i * (lado_cm*scale + espaciado_cm*scale)
                        draw_regular_octagon(ax, cx, y_center, lado_cm*scale, fc="black", ec="white", lw=2.0)
                        # Texto del sello (multilínea si es necesario)
                        txt = tipo_sello
                        ax.text(cx, y_center, txt, ha="center", va="center", color="white", fontsize=9, fontweight="bold", wrap=True)

                    ax.set_xlim(0, 20 + envase_w_cm*scale + 40)
                    ax.set_ylim(0, 20 + envase_h_cm*scale + 40)
                    ax.set_aspect('equal')
                    ax.axis('off')
                    st.pyplot(fig)
